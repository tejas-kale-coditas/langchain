...
def get_patiant_details_tool(patient_name: str, patient_age: int, patient_email: str = None):
    ...
    if patient_name and patient_age:
        try:
            ...
            return 'Invalid patient name format. Please provide both first and last name.'

        for patient in patient_details.get_patients_array():
            try:
                patient_first_name, patient_last_name = patient["name"].split()
            except ValueError:
                continue  # Skip if the patient's name is not in the expected format
...
def process_speech():
    ...
    audio_file = getAudioOfResponse(response[-1])
    return jsonify({"file": audio_file,'response': response[-1]})

ELEVEN_API_KEY= ''

def getAudioOfResponse(text):
    client = ElevenLabs(api_key=ELEVEN_API_KEY)  # Defaults to ELEVEN_API_KEY
    audio = client.generate(
        text=text, voice="EJDYpnH0BA9bzo0Cjrdi", model="eleven_monolingual_v1"
    )
    audio_file = str(uuid.uuid4()) + ".mp3"
    save(audio, "audio/" + audio_file)
    return audio_file
    
from flask import Flask, after_this_request, request, send_from_directory
